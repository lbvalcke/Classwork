...
def flights(itinerary, schedules, t_min, t_max):
    ...
    def mileage_run(itinerary, stop, path, schedules, t_min, t_max):
        start = itinerary[0]
        next_dest = itinerary[1]
        if next_dest in schedules[start].keys():
            if path == []:
                for i, time in enumerate(schedules[start][next_dest]):
                    path.append([time])
                    return mileage_run(itinerary[1:], stop, path, schedules, t_min, t_max)
            else:
                path_dup = []
                for i ,option in enumerate(path):
                    for j, time in enumerate(schedules[start][next_dest]):
                        if t_min <= time[0] - option[-1][1] <= t_max:
                            path_dup.append(option + [time])
                        else: 
                            pass
                if path_dup != []:
                    if itinerary[1] == stop:
                        ...
                        return mileage_run(itinerary[1:], stop, path_dup, schedules, t_min, t_max)


    path = mileage_run(itinerary, itinerary[-1], [], schedules, t_min, t_max)
    path_dup = []
    
    if path != None: 
        for sublist in path:
            path_dup.append(sublist + ["done"])
    else:
        path_dup == None
